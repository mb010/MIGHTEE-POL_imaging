...
def main():
    ...
    if args.copy:
        ...
        if os.path.exists(LOCAL_COPY):
            logger.info(
                "A local copy of the MS already exists at {LOCAL}{TMP_DIR}/{LOCAL_COPY} (likely due to a previous run failing).".format(
                    LOCAL=LOCAL, TMP_DIR=TMP_DIR, LOCAL_COPY=LOCAL_COPY
                )
            )
            logger.info("Deleting local copy before copying over fresh version.")
            shutil.rmtree(LOCAL_COPY)
        logger.info(
            "Copying data to {LOCAL}{TMP_DIR}/ under the name: {LOCAL_COPY}".format(
                LOCAL=LOCAL, TMP_DIR=TMP_DIR, LOCAL_COPY=LOCAL_COPY
            )
        )
